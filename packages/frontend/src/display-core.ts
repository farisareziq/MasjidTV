...
// Teras paparan dikongsi oleh display.ts (lokal) & display-cloud.ts (awan).
// Semua logik dikekalkan verbatim daripada pelaksanaan asal; hanya kod aras
// atas yang BERKESAN-SAMPINGAN (pendengar acara, pemasa, SSE, refresh boot)
// dipindahkan ke dalam bootDisplay(cfg) supaya tiada apa-apa berjalan semasa
// import. Perbezaan varian diparameterkan melalui DisplayVariantConfig.
...
// Konfigurasi varian paparan. Setiap ciri lalai false — varian menghidupkan
// hanya apa yang berbeza daripada pelaksanaan teras (display.ts asal).
export interface DisplayVariantConfig {
  features: {
    // Lokal: debug chip papar status sse:live/poll + jejak 'hello'/'unpaired'
    // pada aliran SSE (server lokal menghantar kedua-dua event itu).
    sseHello?: boolean;
    // Lokal: pada HTTP 503 dengan kod DEVICE_UNPAIRED, reload ke /display
    // (server kini mod pairing dan papar kod baharu).
    unpaired503?: boolean;
    // Awan: domain khas menyuntik kunci melalui <meta name="tvm-key"> —
    // baca meta dan kemas kini URL supaya paparan berfungsi seperti URL berkey.
    metaKey?: boolean;
    // Awan: paparan lama tanpa kunci dalam URL (kiosk/Android TV) muat semula
    // sekali (had 5 minit) untuk pulih sendiri apabila API balas 401.
    recoverMissingKey?: boolean;
...
// Diisi oleh bootDisplay() sebelum sebarang pemasa/acara berjalan; dibaca
// pada masa panggilan (bukan masa import) supaya tiada isu susunan modul.
let cfg: DisplayVariantConfig = { features: {} };
...
function tickDebug(): void {
  ...
  const sseText = cfg.features.sseHello ? ` | sse:${sseAlive ? 'live' : 'poll'}` : '';
  el.textContent = `${innerWidth}×${innerHeight} dpr${devicePixelRatio} | ${phaseInfo}${sseText} | ${boxes.join(' | ')}`;
  el.hidden = false;
}
...
function renderSlides(data: SlideData): void {
  ...
  const mapped: Slide[] = (data.announcements || []).map((a) => ({
    kind: a.category === 'quran' ? ('quran' as const) : (a.category === 'doa' ? ('doa' as const) : (a.video ? ('video' as const) : (a.category === 'tabung' ? ('tabung' as const) : ('announcement' as const)))),
    category: a.category,
    title: a.title,
    message: a.message,
  ...
  // Susunan muktamad: ayat Quran harian → doa harian → pengumuman lain
  // (ikut susunan yang ditetapkan admin) → livestream di hujung sekali
  // kerana kedudukannya tidak boleh diubah-ubah.
  const annSlides: Slide[] = [
  ...
  // Pengumuman + quran + doa (susunan dikunci di atas); livestream sentiasa
...
// Papar mesej baris-me-baris: setiap baris baharu yang dimasukkan admin
// menjadi <p> berasingan; baris kosong berturut-turut diringkan kepada satu
// jarak dan hujung kosong dibuang.
function messageLinesHtml(message: string | null | undefined): string {
  if (!message) return '';
  ...
  // >6 baris: fon dikecilkan (kelas .many) supaya muat dalam slaid.
  const cls = collapsed.filter((l) => l).length > 6 ? ' slide-msgs many' : ' slide-msgs';
  return `<div class="${cls.trim()}">${collapsed
    .map((line) => (line ? `<p class="slide-msg">${escapeHtml(line)}</p>` : '<p class="slide-msg blank"></p>'))
    .join('')}</div>`;
}
...
function renderTicker(data: SlideData): void {
  ...
  if (custom) {
    ...
    // Doa harian dikecualikan (teks arab panjang tidak sesuai marquee);
    // mesej berbilang baris diratakan menjadi satu item ticker sahaja.
    items = data.announcements
      .filter((a) => a.category !== 'doa')
      .map((a) => {
...
// SYNC SEGERA: SSE /api/events — event 'sync' daripada admin memicu sync()
// serta-merta (<1sa). Fallback: poll 10sa kekal jika SSE gagal/legacy.
// Varian lokal turut jejaki 'hello' (status sseAlive dalam chip debug
// ?debug=1 sebagai sse:live/poll) dan 'unpaired' (reload ke pairing).
let sseAlive = false;

// Butirkan paparan: pasang semua pendengar acara, pemasa selang, aliran SSE,
// dan picu refresh pertama. Susunan mengikut kod aras atas asal supaya
// kelakuan boot kekal serupa dengan baseline.
export function bootDisplay(config: DisplayVariantConfig): void {
  cfg = config;

  // (awan sahaja) Domain khas (mis. tvdisplay.masjidlabis.my): server suntik
  // key melalui <meta name="tvm-key"> tanpa skrip inline (CSP ketat). Baca
  // meta dan kemas kini URL supaya paparan berfungsi seperti URL berkey biasa.
  if (cfg.features.metaKey) {
  ...
  // Probe keupayaan playAudio native (asalnya di aras atas modul).
  if (isAndroid) {
    try {
