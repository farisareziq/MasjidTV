...
(() => {
  ...
  var screenName = new URLSearchParams(location.search).get("screen") || "";
  var debugMode = new URLSearchParams(location.search).get("debug") === "1";
  var tz = () => state.settings?.prayer?.timezone || "Asia/Kuala_Lumpur";
  var lang = () => state.settings?.display?.language || "ms";
  var t = (key) => I18N[lang()][key] ?? key;
  var isAndroid = typeof window !== "undefined" && typeof window.AndroidBridge !== "undefined";
  var bridgeMuted = false;
  var sessionInvalidNotified = false;
  var nativeAudioCapable = false;
  var isFriday = () => {
    const tm = testMode();
  ...
  var prayerLabel = (key) => key === "dhuhr" && isFriday() ? t("fridayName") : t("names")[key] || key;
  var fridayJemaahEndMs = () => {
  ...
  function tickDebug() {
    ...
    el.textContent = `${innerWidth}\xD7${innerHeight} dpr${devicePixelRatio} | ${phaseInfo}${sseText} | ${boxes.join(" | ")}`;
    el.hidden = false;
  }
  ...
  var ADHAN_CALL_MS = 7e3;
  var IQAMAH_LEAD_MS = 3e4;
  ...
  var dataCache = { settings: "", today: "", slides: "" };
  var stableToday = (today) => {
...
`;
  ...
  // packages/frontend/src/display-cloud.ts
  bootDisplay({
    features: {
      metaKey: true,
