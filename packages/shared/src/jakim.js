...
const WEEK_INFLIGHT = new Map();
let syncRunning = false;
async function apiFetch(zone, period, { start, end } = {}, attempt = 1) {
...
function parseEntry(raw) {
    ...
    const hhmm = (s) => (typeof s === 'string' && s.length >= 5 ? s.slice(0, 5) : null);
    return {
        dateKey,
        hijri,
        day: String(raw.day ?? ''),
...
async function getWeekEntries(zone) {
    ...
    if (cached && cached.dateKey === parts)
    ...
    const fetchPromise = apiFetch(zone, 'week')
        .then((json) => {
        const entries = (json.prayerTime || []).map((e) => parseEntry(e)).filter((e) => e !== null);
...
export async function getEntryForDate(zone, dateKey) {
...
async function fetchRange(zone, startKey, endKey) {
    const all = [];
    let cursor = startKey;
    while (cursor <= endKey) {
        ...
        all.push(...(json.prayerTime || []).map((e) => parseEntry(e)).filter((e) => e !== null));
        if (end === endKey)
            break;
...
function approximateNextOccurrence(month, day, now) {
...
export async function syncEventsFor(settings, saveFn, force = false) {
    if (syncRunning)
        return { ok: false, message: 'Sync sedang berjalan' };
    if (!settings.eventsSync?.enabled && !force)
        return { ok: false, message: 'Auto-sync dimatikan' };
    syncRunning = true;
    try {
        const zone = settings.prayer.zone;
        if (!getZone(zone))
            throw new Error(`Zon tidak sah: ${zone}`);
        const now = new Date();
        ...
        const byHijri = new Map();
        for (const e of entries) {
            if (e.hijri)
                byHijri.set(`${e.hijri.month}-${e.hijri.day}`, e.dateKey);
        }
        const synced = [];
        const approximated = [];
        ...
        if (!synced.length && !approximated.length)
            throw new Error('Tiada tarikh diperoleh dari JAKIM');
...
export function hijriText(hijri) {
    if (!hijri)
        return null;
