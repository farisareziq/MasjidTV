...
function islamicToRd(year, month, day) {
    return day + Math.ceil(29.5 * (month - 1)) + (year - 1) * 354
        + intDiv(3 + 11 * year, 30) + ISLAMIC_EPOCH_RD - 1;
}
function rdToIslamic(rd) {
...
export function hijriForDateKey(dateKey, offsetDays = 0) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(String(dateKey || '')))
        return null;
    const [y, m, d] = String(dateKey).split('-').map(Number);
    const rd = gregorianToRd(y, m, d) + (Number(offsetDays) || 0);
    const h = rdToIslamic(rd);
    if (!h.year || h.month < 1 || h.month > 12 || h.day < 1 || h.day > 30)
        return null;
    return h;
}
