...
export function createLocalClient(filePath: string): LocalClient {
...
export function createMemoryClient(): LocalClient {
  return createLocalClient(':memory:');
}

// Cloud client (Turso/libsql). `url` may be libsql://, https://, or file:.
export function createCloudClient(url: string, authToken?: string): CloudClient {
